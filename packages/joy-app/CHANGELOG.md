...
- **Tapping into the message box no longer shifts the chat.** When the area holding the chat changes height — the box growing to a second line, the keyboard on Android — the list now re-pins its bottom in the same layout pass instead of letting the rows slide and snapping them back a moment later.
...
- **What this gives up.** A bare link in your own message is no longer tappable, because nothing in it is being parsed any more. Links in an agent's replies are unaffected, and search still marks matches inside your messages.

# Sep 17 (2) — When the app and a machine disagree

- **The out-of-date warning now names the side that is actually behind.** It could only ever say to update the machine, because it only checked whether a machine's daemon was too old. A machine running something *newer* than the app understands is just as much a mismatch, and it is the more common one: a machine updates in one step, while the app waits on a store review or a published update. The pill above a session now tells you which one to update. Dismissing it still only silences that exact version, so it returns by itself when either side moves.

# Sep 17 (1) — Scrolling up during a reply stays up

- **Scrolling back through a chat while an agent is typing no longer yanks you to the bottom.** A small scroll up was answered by a jump back down, and then again on the next batch of text, for as long as the reply continued. Following now pays attention to direction rather than distance alone: moving up leaves the live view, text arriving beneath you is not mistaken for you moving, and scrolling back down resumes following.
- **On desktop, the wheel counts as your hand on the page.** A mouse wheel produces none of the touch signals a phone does, so the guard that pauses following while you are moving was never switched on there. That is why it was worst with a mouse.
