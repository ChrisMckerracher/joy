...
export function createPocketSpeech() {
    ...
    let nextId = 0;
    const pending = new Map<number, { resolve: (value: Uint8Array) => void; reject: (error: Error) => void }>();
    let disposed = false;
    ...
    const request = (type: string, payload: object) => new Promise<Uint8Array>((resolve, reject) => {
    ...
    return {
        ...
        async generate(text: string, signal: AbortSignal) {
            ...
            try { return await request('generate', { text }); }
